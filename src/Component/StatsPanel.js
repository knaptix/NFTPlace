...
const StatsPanel = () => {
  ...
  return (
    <div className="w-full bg-[#d4d3d3] py-6 px-4 mb-10">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
        {stats.map((stat) => (
          <div
            key={stat.id}
            className="flex flex-col items-center md:flex-row md:items-center space-y-4 md:space-y-0 md:space-x-4 p-6"
          >
            <div className="p-3 bg-blue-50 rounded-lg">
              <img src={stat.image} alt={stat.label} className="w-16 h-16 bg-[#D3D3D3]" />
            </div>
            <div className="text-center md:text-left">
              <div className="text-[24px] md:text-[32px] font-bold font-sans text-gray-900">
                {stat.value}
              </div>
